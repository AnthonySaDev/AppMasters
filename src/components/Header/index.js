...
export default function Header() {
  ...
  return (
<header className={`top-0 font-sans fixed w-full text-white text-xs tracking-widest z-50 items-center flex px-8 justify-around py-5 ${isScrolled ? 'bg-[#020076] transition-all duration-500 ease-in-out' : ''}`} style={{backgroundImage: ` transparent`}}>
      <div className="flex items-center gap-8 w-full md:px-5 justify-between">
        ...
        >
          <a href="/" className={`${isScrolled ? 'brightness-[4]' : '  transition-all duration-300'}`}>
        ...
        <motion.nav
          ...
          <a className="hover:text-yellow-600 transition-colors duration-300 w-fit whitespace-nowrap flex items-center gap-2" href="#cards">
...
.showMenuNav {
  ...
  right: 0rem;
  color: white;
  background: ${isScrolled ? '#020076' : 'transparent'};
  z-index: 99;
